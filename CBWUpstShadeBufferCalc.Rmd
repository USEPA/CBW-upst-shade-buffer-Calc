...
```{r, eval=F, warning=FALSE, message=FALSE}
shade_coords <- readRDS("./data/RShade Preprocessing/HUC0205/Output_0205.RDS") %>% select(site_id) %>% mutate(HUC = 0205) 

shade_coords <- 
  rbind(
    shade_coords,
    rbind(
      fread("./data/RShade Preprocessing/HUC0206/Output.csv") %>% mutate(HUC = 0206),
      fread("./data/RShade Preprocessing/HUC0207/Output.csv") %>% mutate(HUC = 0207),
      fread("./data/RShade Preprocessing/HUC0208/Output.csv") %>% mutate(HUC = 0208)) %>%
...
FOR TEST: load in the example dataframes, otherwise skip.
